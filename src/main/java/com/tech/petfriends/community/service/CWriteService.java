...
@Service
	public class CWriteService implements CServiceMInterface {
		...
		public void execute(Model model) {
		    ...
		    // Jsoup을 사용하여 <script> 태그를 제거하고, 다른 태그는 기본적으로 허용
		    org.jsoup.nodes.Document doc = Jsoup.parse(board_content);
		    doc.select("script").remove(); // 모든 <script> 태그 제거
		    String safeBoardContent = Jsoup.clean(doc.html(), Whitelist.basic().addTags("p", "b", "i", "u", "ul", "ol", "li", "a", "img"));
		    ...
		    org.jsoup.nodes.Document doc3 = Jsoup.parse(board_content_input);
		    doc.select("script").remove(); // 모든 <script> 태그 제거
		    String safeBoardcontent_input = Jsoup.clean(doc.html(), Whitelist.basic().addTags("p", "b", "i", "u", "ul", "ol", "li", "a", "img"));
		    
		    
		    // 게시글 작성
		    iDao.write(mem_nick, mem_code, board_title, safeBoardContent, b_cate_no, safeBoardcontent_input);
